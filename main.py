...
if __name__ == '__main__':
    ...
    parser.add_argument('--last_backward_length', '-b', type=int, default=1, help='parking_margin [m]')
    ...
    path = path_planner.plan_path(
        int(start[0] - my_car.a / 2 * np.cos(np.deg2rad(args.psi_start))),
        int(start[1] - my_car.a / 2 * np.sin(np.deg2rad(args.psi_start))),
        np.deg2rad(args.psi_start),
        int(end[0]), int(end[1]))
